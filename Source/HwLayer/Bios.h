...
public:
  ...
  public:
    static void Init();
    static int Printf(int x, int y, unsigned short clrf, unsigned short clrb,
                      const char *format, ...);
    static int Print(int x, int y, unsigned short clrf, unsigned short clrb,
                     char *str);
    static int Print(int x, int y, unsigned short clrf, unsigned short clrb,
                     const char *str);
    ...
    static int Draw(int x, int y, unsigned short clrf, unsigned short clrb,
                    const char *p);
  ...
  public:
    ...
    enum EState { Start = 0, Empty = 1, Full = 2 };

    enum { Length = 4096, BufferLength = Length * 4 };
  ...
  public:
    ...
    static bool Enabled();
    static void Configure(ui8 nACouple, ui8 nARange, ui16 nAOffset,
                          ui8 nBCouple, ui8 nBRange, ui16 nBOffset,
                          float fTimePerDiv);
    static void ConfigureTrigger(ui16 nTThreshold, ui16 nVThreshold,
  ...
  class DLLAPI DSK {
  public:
    enum { IoRead = 1, IoWrite = 2, IoClosed = 3 };
  ...
  class DLLAPI FAT {
  public:
    enum EResult { EOk, EDiskError, EIntError, ENoFile, ENoPath, EDiskFull };
