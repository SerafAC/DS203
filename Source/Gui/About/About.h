...
//		BIOS::LCD::Printf(20, 36, RGB565(ff0000), 0, "%d %d", nA, nB);
  ...
  virtual void OnPaint() {
    ...
    BIOS::LCD::Print(160, 240 - 7 * 16, clrB, 0,
                     BIOS::VER::GetHardwareVersion());
    ...
    BIOS::LCD::Printf(160, 240 - 2 * 16, clrB, 0, "%08x",
                      BIOS::VER::GetSerialNumber());

    // logo 256x64
    BIOS::VER::DrawLogo((m_rcClient.Width() - 256) / 2,
                        m_rcClient.top + 8 + 16);
  }
};
...
#ifdef GIT_REVISION
    BIOS::LCD::Printf(160, 240 - 7 * 16, clrB, 0, "%a rev. %d", strVersion,
                      GIT_REVISION);
#else
...
#ifdef __GNUC__
    BIOS::LCD::Printf(160, 240 - 3 * 16, clrB, 0, "GCC %d.%d.%d", __GNUC__,
...
public:
  ...
  virtual void OnTimer() {
    ...
    BIOS::LCD::Printf(240, 240 - 10 * 16, clrB, 0, "%s  ",
                      nUsbPower ? "Yes" : "No");
    BIOS::LCD::Printf(240, 240 - 12 * 16, clrB, 0, "%s  ",
                      BIOS::SYS::IsColdBoot() ? "Yes" : "No");
  }
};
  ...
  virtual void OnPaint() {
    ...
    for (int i = BIOS::SYS::EApp1; i <= BIOS::SYS::EApp4; i++) {
      ...
      if (strName && strcmp(strName, "Unknown") == 0)
        BIOS::LCD::Printf(4, y, clrA, 0, "APP%d (valid vector table found)",
                          i - BIOS::SYS::EApp1 + 1);
      else if (strName)
        BIOS::LCD::Printf(4, y, clrA, 0, "APP%d (identification found at 0x%x)",
                          i - BIOS::SYS::EApp1 + 1, strName);
      else
        BIOS::LCD::Printf(4, y, clrA, 0, "APP%d (identification not found)",
                          i - BIOS::SYS::EApp1 + 1);
      ...
      /*
                                      ...
                                      switch(i)
                                      {
                                              case BIOS::SYS::EApp1: dwAddr =
         0x0800C000; break;
                                              case BIOS::SYS::EApp2: dwAddr =
         0x08014000; break;
                                              case BIOS::SYS::EApp3: dwAddr =
         0x0801C000; break;
                                              case BIOS::SYS::EApp4: dwAddr =
         0x08024000; break;
                                      }
                                      ui32* pData = (ui32*)dwAddr;

                                      BIOS::LCD::Printf(4,y+32, 0xffff, 0,
