...
void CWndListCalSimple::LoadCalib() {
  ...
  _ASSERT((int)m_calRange >= 0 &&
          (int)m_calRange < COUNT(Settings.CH1Calib.CalData));
  ...
  /*
                  void Prepare(int nRes, int nVert, LinCalibCurve& pCurCurve,
     FastCalc& fast)
                  {
     ...
     pCurCurve.m_arrCurveQout, nVert );
                  ...
                  float Voltage(FastCalc& fast, float fAdc)
                  {
                          ...
                          Y = (X+a)*b = a*b + X*b
                          return (fast.Q + fAdc * fast.K) * fast.fMultiplier /
     65536.0f;
                  }
                  */
}
...
void CWndListCalSimple::UpdateZero() {
  BIOS::DBG::sprintf(m_strQ0, " Vavg(%d)    ", m_calCurve.m_arrCurveQin[0]);
  BIOS::DBG::sprintf(m_strQ0 + 10, " = %1f  ", m_nQ0 / -2048.0f);

  m_itmStaticB.m_pszId = m_strQ0;
  m_itmStaticB.Invalidate();

  BIOS::DBG::sprintf(m_strQ1, " Vavg(%d)    ", m_calCurve.m_arrCurveQin[1]);
  BIOS::DBG::sprintf(m_strQ1 + 10, " = %1f  ", m_nQ1 / -2048.0f);
...
void CWndListCalSimple::OnWave() {
  ...
  if (m_nResetPhase == -1) {
    if (nSum == 256 * 255)
      ...
      BIOS::DBG::sprintf(m_strV, " Vin       = %s ",
                         CUtils::FormatVoltage(fValue));
...
/*virtual*/ void CWndListCalSimple::OnMessage(CWnd *pSender, CodeParam code,
                                              DataParam data) {
  ...
  if (pSender == &m_itmReset && code == ToWord('l', 'e')) {
    MainWnd.m_wndConfirm.Show(this, "Message",
                              "Connect probe to ground\nReady?", RGB565(ffff00),
                              "Yes", "No");
    return;
  }
