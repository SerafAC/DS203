...
/*virtual*/ void CWndMenuSpectMarker::Create(CWnd *pParent, ui16 dwFlags) {
  CWnd::Create("CWndMenuSpectMain", dwFlags | CWnd::WsListener,
               CRect(316 - CWndMenuItem::MarginLeft, 20, 400, 240), pParent);
...
/*virtual*/ void CWndMenuSpectMarker::OnMessage(CWnd *pSender, CodeParam code,
                                                DataParam data) {
  ...
  if (code == ToWord('L', 'E')) {
    MainWnd.m_wndSpectrumMiniTD.ShowWindow(
        (Settings.Spec.Display == CSettings::Spectrum::_FftTime ||
         Settings.Spec.Display == CSettings::Spectrum::_Spectrograph)
            ? SwShow
            : SwHide);
    MainWnd.m_wndSpectrumMiniFD.ShowWindow(
        Settings.Spec.Display == CSettings::Spectrum::_FftTime ? SwShow
                                                               : SwHide);
    MainWnd.m_wndSpectrumGraph.ShowWindow(
        Settings.Spec.Display == CSettings::Spectrum::_Fft ? SwShow : SwHide);
    MainWnd.m_wndSpectrumMiniSG.ShowWindow(
        Settings.Spec.Display == CSettings::Spectrum::_Spectrograph ? SwShow
                                                                    : SwHide);
