...
public:
  ...
  virtual void OnPaint() {
    ...
    BIOS::LCD::Print(x, y, clr, RGBTRANS,
                     CUtils::itoa(Settings.Spec.nWindowLength));
  }
  /*
          virtual void OnKey(ui16 nKey)
          {
                  if ( nKey & BIOS::KEY::KeyLeft && (*m_pProvider)-1 ==
     CValueProvider::Yes )
                  {
                          (*m_pProvider)--;
                          Invalidate();
                  ...
                  if ( nKey & BIOS::KEY::KeyRight && (*m_pProvider)+1 ==
     CValueProvider::Yes )
                  {
                          (*m_pProvider)++;
                          Invalidate();
                  ...
                  if ( nKey & BIOS::KEY::KeyEnter  )
                  {
                          ...
                          if ( m_pProvider->Get() !=
     CValueProvider::Invalid )
                          {
                                  SendMessage(m_pParent, ToWord('l', 'e'),
     (m_pProvider));
                          }
                  }
