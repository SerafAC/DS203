...
/*virtual*/ void CWndSpectrumGraphTempl::Create(CWnd *pParent, ui16 dwFlags) {
  // CWnd::Create("CWndSpectrumGraph", dwFlags | CWnd::WsListener, CRect(34, 22,
  // 34+DivsX*BlkX, 22+DivsY*BlkY), pParent);
  CWnd::Create(
      "CWndSpectrumGraph", dwFlags | CWnd::WsListener,
      CRect(34 + 16, 222 - DivsY * m_nBlkY, 34 + 16 + DivsX * m_nBlkX, 222),
      pParent);
}
...
void CWndSpectrumGraphTempl::OnPaint() {
  ...
  if (nMarkerX == nLength) {
    ...
    Settings.Spec.fMarkerX = nMarkerX / 256.0f *
                             (1.0f / (Settings.Runtime.m_fTimeRes / 30.0f)) *
                             0.5f;
    Settings.Spec.fMarkerY =
  ...
  for (i = 0; i < DivsX * m_nBlkX; i++) {
    ...
    BIOS::LCD::Buffer(m_rcClient.left + i, m_rcClient.top, column,
                      DivsY * m_nBlkY);
    if (i == nMarkerX + 3) {
      BIOS::LCD::Draw(m_rcClient.left + nMarkerX - 3,
                      m_rcClient.bottom - nMarkerY - 4, RGB565(ff0000),
                      RGBTRANS, CShapes::markerX);
    }
  }
}
...
/*virtual*/ void CWndTimeGraphTempl::Create(CWnd *pParent, ui16 dwFlags) {
  CWnd::Create(
      "CWndTimeGraphTempl", dwFlags | CWnd::WsListener,
      CRect(34 + 16, 22, 34 + 16 + DivsX * m_nBlkX, 22 + DivsY * m_nBlkY),
      pParent);
}
...
#define LineTo(buf, ynew, ylast, clr)                     \
  ...
  for (i = 0; i < nPixels; i++) {
    ...
    for (int nTest = nSampleIndex; nTest < nCurSampleIndex && !bSatur;
         nTest++) {
      BIOS::ADC::SSample Sample;
...
/*virtual*/ void CWndSpectrographTempl::Create(CWnd *pParent, ui16 dwFlags) {
  CWnd::Create("CWndSpectrographTempl", dwFlags | CWnd::WsListener,
               CRect(34 + 16, 222 - m_nHeight, 34 + 16 + m_nWidth, 222),
               pParent);
  m_nY = 0;
}
...
/*virtual*/ void CWndSpectrographTempl::OnPaint() {
  ...
  for (int i = 0; i < 256; i++) {
    ...
    ui8 color[3] = {Get565R(clrBackground), Get565G(clrBackground),
                    Get565B(clrBackground)};
    if (en1) {
      int colorA[3] = {(int)((GetColorR(clr32A) * ch1) >> 8),
    ...
    if (en2) {
      int colorB[3] = {(int)((GetColorR(clr32B) * ch2) >> 8),
  ...
  // blit
  BIOS::LCD::PutImage(CRect(m_rcClient.left, m_rcClient.top + m_nY,
