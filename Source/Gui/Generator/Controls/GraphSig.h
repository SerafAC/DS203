...
public:
  ...
  void ExitHandler(CWnd *pExit) { m_pExit = pExit; }
  ...
  virtual void OnPaint() {
    if (!m_rcRange.IsValid()) {
      ...
      for (int x = 1; x < 10; x++)
        BIOS::LCD::Pattern(m_rcClient.left + BlkX * x, m_rcClient.top,
                           m_rcClient.left + BlkX * x + 1,
                           m_rcClient.bottom - 4, grid, COUNT(grid));
      for (int y = 1; y < 8; y++)
        BIOS::LCD::Pattern(m_rcClient.left, m_rcClient.top + BlkY * y,
                           m_rcClient.right, m_rcClient.top + BlkY * y + 1,
                           grid, COUNT(grid));
    }
    ...
    ui16 oy = 0;
    int nSelected =
        HasFocus() ? m_nEditX * nWidth / m_nCount + nWidth / m_nCount / 2 : -20;
    int nSelectedY = 0;
    ...
    for (ui16 i = m_rcRange.left; i < m_rcRange.right; i++) {
      ...
      if (i == nSelected + 3)
        BIOS::LCD::Draw(m_rcClient.left + i - 6, nBaseY - nSelectedY - 4,
                        (m_nStep == 8) ? RGB565(ff0000) : RGB565(b00000),
  ...
  virtual void OnKey(ui16 nKey) {
    ...
    if (nKey & BIOS::KEY::KeyRight) {
      if (m_nEditX < CCoreGenerator::GetVolatileLen() - 1) {
        ...
        m_rcRange.right =
            m_rcRange.left + 2 * m_rcClient.Width() / m_nCount + 4;
        m_nEditX++;
        Invalidate();
      }
      return;
