...
/*virtual*/ void CWndMenuGenerator::Create(CWnd *pParent, ui16 dwFlags) {
  ...
  CWnd::Create("CWndMenuGenerator", dwFlags,
               CRect(280 - CWndMenuItem::MarginLeft, 20, 400, 240), pParent);
...
#define MHz *1000000
  ...
  if (code == ToWord('L', 'E')) {
    ...
    bool bShowAmpl =
        (Settings.Gen.Wave != CSettings::Generator::_Dc) && bShowOffset;
    ...
    if (Settings.Gen.Wave == CSettings::Generator::_Dc)
      ...
      MainWnd.m_wndSignalGraph.Setup(
          CCoreGenerator::GetWave(Settings.Gen.Wave)->pWave,
          CCoreGenerator::GetWave(Settings.Gen.Wave)->nCount);
    } else
      MainWnd.m_wndSignalGraph.Setup(CCoreGenerator::GetRamDac(),
                                     CCoreGenerator::GetRamLen());
    return;
  }
