...
#define FOC(n) (m_nFocus == n) ? RGB565(ffffff) : RGB565(808080)
  PrintBold(m_rcClient.left + 8, m_rcClient.top + 2 + 0 * 16, FOC(0),
            RGB565(000000), "\x10 Waveform manager");
  PrintBold(m_rcClient.left + 8, m_rcClient.top + 2 + 1 * 16, FOC(1),
            RGB565(000000), m_bAdcEnabled ? "\x10 Pause" : "\x10 Resume");
  PrintBold(m_rcClient.left + 8, m_rcClient.top + 2 + 2 * 16, FOC(2),
            RGB565(000000), "\x10 Reset setings");
  PrintBold(m_rcClient.left + 8, m_rcClient.top + 2 + 3 * 16, FOC(3),
            RGB565(000000), "\x10 Save setings");
  ...
  BIOS::LCD::Printf(m_rcClient.left + 8, m_rcClient.bottom - 16, RGB565(000000),
                    RGBTRANS, "\x10 Select  \xfe Close");
  PrintBold(m_rcClient.left + 8 + 120, m_rcClient.bottom - 16 - 18,
            RGB565(ffff00), RGB565(000000), str);
  // BIOS::LCD::Printf( m_rcClient.left + 8 + 130, m_rcClient.top + 8,
  // RGB565(000000), RGBTRANS, str);
}

void CWndToolbox::PrintBold(int x, int y, ui16 clrFront, ui16 clrBorder,
                            PCSTR szLabel) {
...
/*virtual*/ bool CWndToolbox::IsRunning() { return m_bRunning; }
...
/*virtual*/ int CWndToolbox::GetResult() { return m_nFocus; }
