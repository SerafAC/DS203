#pragma once
#ifndef DSO_GUI_TOOLBOX_EXPORT_H
#define DSO_GUI_TOOLBOX_EXPORT_H
...
#pragma pack(pop)
 ...
 public:
  ...
  void SaveBinary(char *strName_ = NULL) {
    ...
    for (int i = 0; i < /*BIOS::ADC::Length()*/ (int)BIOS::ADC::GetCount();
         i++) {
  ...
  void SaveCsv(char *strName_ = NULL) {
    ...
    for (int i = 0; i < (int)BIOS::ADC::GetCount(); i++) {
      ...
      float fTime =
          fTimeRes * (i - (Settings.Trig.nTime - Settings.Time.Shift));
