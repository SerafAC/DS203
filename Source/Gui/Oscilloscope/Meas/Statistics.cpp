...
bool CMeasStatistics::_GetEffectiveValuesForPower(float &fVoltage,
                                                  float &fCurrent) {
...
int CMeasStatistics::_FindEdge(CSettings::Measure::ESource src,
...
float CMeasStatistics::GetChannelsDelta(bool rising) {
  ...
  for (int i = nBegin; i < nEnd; i++) {
    int edge =
        _FindEdge(src, m_curRange, rising ? 1 : -1, i, nTrigStateChannel);

    if (edge > 0) {
      int edge2 = _FindEdge(CSettings::Measure::_Math, m_curRange,
                            rising ? 1 : -1, edge, nTrigStateMath);
      if (edge2 > edge) {
        nSum += edge2 - edge;
        nTotal++;
      }
...
float CMeasStatistics::GetBaud() {
  ...
  bool bCan100k =
      (fBaud > 100000.0f - 5000.0f) && (fBaud < 100000.0f + 5000.0f);
  bool bCan500k =
      (fBaud > 500000.0f - 5000.0f) && (fBaud < 500000.0f + 5000.0f);
...
float CMeasStatistics::GetRms() {
  return narrow_cast<float>(sqrt(m_fSum2 / m_nCount));
}
...
float CMeasStatistics::GetTime(bool bHighLevel) {
  ...
  for (int i = nBegin; i < nEnd; i++) {
    int edge =
        _FindEdge(src, m_curRange, bHighLevel ? 1 : -1, i, nTrigStateChannel);

    if (edge > 0) {
      int edge2 =
          _FindEdge(src, m_curRange, bHighLevel ? -1 : 1, edge, nTrigStateMath);
      if (edge2 > edge) {
        nSum += edge2 - edge;
        nTotal++;
...
float CMeasStatistics::GetDispersion() {
  float f = GetSigma();
  return f * f;
}
