...
public:
  ...
  bool Sum(int nCount, int nRising) {
    ...
    nTotalBits += nBits;
    nTotalTime += nCount;
    // BIOS::DBG::Print( "rel=%f %d/%d => %d\n", nCount/(float)nMinPeriod,
    // nCount, nMinPeriod, nBits);
    return true;
  }
  ...
  void SetDecoder(CBitDecoder *pDecoder) { m_pCurrentDecoder = pDecoder; }
  ...
  float GetBaud() {
    float fBaudPeriod = (nTotalTime / (float)nTotalBits) *
                        Settings.Runtime.m_fTimeRes / CWndGraph::BlkX;
  ...
  void _ShowMidi(int x, int y) {
    for (int i = 0; i < nDecoded; i++) {
      ...
      if ((ch & 0xf0) == 0x90) {
        if (atr2 == 0)
          x +=
              8 * BIOS::LCD::Printf(x, y, RGB565(ff0000), 0x0101, "NoteOff(%s)",
                                    CUtils::MidiNote(atr1), atr2);
        else
          x += 8 * BIOS::LCD::Printf(x, y, RGB565(ff0000), 0x0101,
                                     "NoteOn(%s,%d)", CUtils::MidiNote(atr1),
                                     atr2);
        i += 2;
      }
      if ((ch & 0xf0) == 0x80) {
