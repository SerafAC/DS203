...
void CMathChannel::MathSetup(CSettings::Calibrator::FastCalc *pFC1,
  ...
  if (Settings.Math.Type == CSettings::MathOperator::_Off) {
    ...
    MainWnd.m_wndMessage.Show(&MainWnd, "Information",
                              "Math operator set to A+B+C", RGB565(ffff00));
  }
...
int CMathChannel::MathCalc(ui32 nSample) {
  ...
  switch (Settings.Math.Type) {
    ...
    case CSettings::MathOperator::_AplusBplusC:
      res = MathGet(Settings.MathA, nSample) +
            MathGet(Settings.MathB, nSample) + MathGet(Settings.MathC, nSample);
      break;
    ...
    case CSettings::MathOperator::_BminusAplusC:
      res = MathGet(Settings.MathB, nSample) -
            MathGet(Settings.MathA, nSample) + MathGet(Settings.MathC, nSample);
      break;
    case CSettings::MathOperator::_AgreaterBplusC:
      res = (MathGet(Settings.MathA, nSample) > MathGet(Settings.MathB, nSample)
                 ? 32
                 : 0) +
...
int CMathChannel::MathGet(CSettings::MathOperand &op, int nSample) {
  ...
  switch (op.Type) {
    case CSettings::MathOperand::_CH1Corrected:
      ...
      if (op.nScale != 100) nAux = nAux * op.nScale / 100;
      nAux = (int)(nAux * m_pFC1->fMultiplier /
    ...
    case CSettings::MathOperand::_Ch2Corrected:
      ...
      if (op.nScale != 100) nAux = nAux * op.nScale / 100;
      nAux = (int)(nAux * m_pFC2->fMultiplier /
                   Settings.CH2Calib.GetMultiplier(Settings.Math.Resolution));
...
int CMathChannel::MathFx(int nParam, int nScale) {
  ...
  int nValue = (int)(128 + 127 * nScale / 100.0f *
                               sin(2 * 3.141592653f * nParam *
                                   (m_nTemp / (float)nTotal)));
#ifdef _WIN32
