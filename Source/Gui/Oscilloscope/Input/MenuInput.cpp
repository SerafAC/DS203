...
/*virtual*/ void CWndMenuInput::Create(CWnd *pParent, ui16 dwFlags) {
  CWnd::Create("CWndMenuInput", dwFlags,
...
LINKERSECTION(".extra")
/*virtual*/ void CWndMenuInput::OnMessage(CWnd *pSender, CodeParam code,
                                          DataParam data) {
  ...
  if (code == ToWord('u', 'p') && pSender->m_pParent == &m_wndListTrigger) {
    ...
    Settings.Trig.State = (Settings.Trig.Sync == CSettings::Trigger::_Single)
                              ? CSettings::Trigger::_Wait
                              : CSettings::Trigger::_Run;
    ;
  ...
  if (code == ToWord('l', 'e')) // provider selector combo box
  {
    ...
    if ( pProvider->Get() == 0 )
    {
            ...
            // button ?
            MainWnd.m_wndMessage.Show( this, "Message",
    ((CProviderBtn*)pProvider)->m_pName, RGB565(ffff00));
            return;
    } else
    {*/
    m_wndComboSelector.Create(GetFocus()->m_pszId, WsVisible | WsModal,
