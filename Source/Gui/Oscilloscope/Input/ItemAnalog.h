...
class CItemAnalog : public CWndMenuItem {
 ...
 public:
  ...
  virtual void OnPaint() {
    ui16 clr = (m_pInfo->Enabled == CSettings::AnalogChannel::_YES)
                   ? RGB565(000000)
                   : RGB565(808080);
  ...
  virtual void OnKey(ui16 nKey) {
    ...
    if (nKey & BIOS::KEY::KeyRight &&
        m_pInfo->Resolution < CSettings::AnalogChannel::_ResolutionMax) {
      _ASSERT(sizeof(m_pInfo->Resolution) == sizeof(NATIVEENUM));
    ...
    if (nKey & BIOS::KEY::KeyEnter) {
      SendMessage(m_pParent, ToWord('m', 'a'), m_pInfo);
    }
    CWnd::OnKey(nKey);
  }
};
