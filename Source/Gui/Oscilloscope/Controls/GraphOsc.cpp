...
/*virtual*/ void CWndOscGraph::OnMessage(CWnd *pSender, CodeParam code,
                                         DataParam data) {
...
void CWndOscGraph::OnPaintXY() {
  ...
  if (Settings.Disp.Persist == CSettings::Display::_PerNo || m_bPersistReset ||
      m_bNeedRedraw)
  ...
  if (!bEnabled1 || !bEnabled2) {
    BIOS::LCD::Print(m_rcClient.left, m_rcClient.top, RGB565(ffffff), RGBTRANS,
                     (char *)"Enable both channels");
    return;
  }
  ...
  for (ui16 x = 0; x < nColMax; x++, nIndex++) {
    ...
    if (Settings.Disp.Axes == CSettings::Display::_XY)
      BIOS::LCD::PutPixel(m_rcClient.left + nSampleY1 + BlkX,
                          m_rcClient.bottom - nSampleY2, clrPoint);
    else
      BIOS::LCD::PutPixel(m_rcClient.left + nSampleY2 + BlkX,
                          m_rcClient.bottom - nSampleY1, clrPoint);
  }
}
...
void CWndOscGraph::OnPaintTY() {
  ...
  if (CWnd::GetOverlay().IsValid() &&
      CWnd::GetOverlay().left - m_rcClient.left <= 0)
    return;
  ...
  for (ui16 x = 0; x < nMax; x++, nIndex++) {
    ...
    if (bUsingMask) {
      if (x == 80 &&
          MainWnd.m_wndMenuMask.m_Display != CWndMenuMask::DisplayNo) {
        int *nPass = NULL;
        int *nFail = NULL;
        ...
        BIOS::LCD::Printf(m_rcClient.left + 2, m_rcClient.bottom - 16,
                          RGB565(ffffff), 0x0101, "%1f%%",
                          (*nPass) * 100.0f / (*nFail + *nPass));
      }
      ui8 *pLow = NULL;
      ui8 *pHigh = NULL;
    ...
    if (x <= nCut)
      ...
      if (nCutTop > 0)
        BIOS::LCD::Buffer(m_rcClient.left + x, m_rcClient.top + nCutTop, column,
                          DivsY * BlkY - nCutTop);
      if (nFirstTop > 0)
        BIOS::LCD::Buffer(m_rcClient.left + x, m_rcClient.top,
                          column + DivsY * BlkY - nFirstTop, nFirstTop);
    }
  }
}

void CWndOscGraph::SetupSelection(bool &bSelection, int &nMarkerT1,
                                  int &nMarkerT2) {
...
void CWndOscGraph::SetupMarkers(CSettings::Calibrator::FastCalc &Ch1fast,
                                CSettings::Calibrator::FastCalc &Ch2fast,
                                int &nMarkerT1, int &nMarkerT2, int &nMarkerY1,
                                int &nMarkerY2) {
...
void CWndOscGraph::ClearPersist() { m_bPersistReset = true; }
