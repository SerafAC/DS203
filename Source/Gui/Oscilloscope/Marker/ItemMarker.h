#pragma once
#ifndef DSO_GUI_OSCILLOSCOPE_MARKER_ITEMMARKER_H
#define DSO_GUI_OSCILLOSCOPE_MARKER_ITEMMARKER_H
...
class CItemMarker : public CWndMenuItem {
 ...
 public:
  ...
  virtual void OnPaint() {
    ui16 clr = (m_pMarker->Mode != CSettings::Marker::_Off) ? RGB565(000000)
                                                            : RGB565(808080);
    ui16 clr2 = (m_pMarker->Mode != CSettings::Marker::_Off) ? RGB565(404040)
                                                             : RGB565(909090);
    ...
    if (m_pMarker->Display == CSettings::Marker::_Raw)
      x += BIOS::LCD::Print(x, y, clr, RGBTRANS,
                            CUtils::itoa(m_pMarker->nValue));
    else {
      if (m_pMarker->Type == CSettings::Marker::_Time) {
        float fTimeRes = Settings.Runtime.m_fTimeRes / CWndGraph::BlkX;
        float fValue = fTimeRes * (m_pMarker->nValue -
                                   (Settings.Trig.nTime - Settings.Time.Shift));
      ...
      if (m_pMarker->Type == CSettings::Marker::_Voltage) {
        ...
        float fValue =
            Settings.CH1Calib.Voltage(fastCalc, (float)m_pMarker->nValue);
  ...
  virtual void OnKey(ui16 nKey) {
    ...
    if (nKey & BIOS::KEY::KeyRight) {
      if (m_pMarker->Type == CSettings::Marker::_Time &&
          m_pMarker->nValue < 1024)
        m_pMarker->nValue++;
      if (m_pMarker->Type == CSettings::Marker::_Voltage &&
          m_pMarker->nValue < 256)
        m_pMarker->nValue++;
