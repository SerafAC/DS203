...
/*virtual*/ void CWndMenuCursor::Create(CWnd *pParent, ui16 dwFlags) {
  CWnd::Create("CWndMenuCursor", dwFlags | CWnd::WsListener,
...
/*virtual*/ void CWndMenuCursor::OnMessage(CWnd *pSender, CodeParam code,
                                           DataParam data) {
  ...
  if (code == ToWord('u', 'p')) {
    if (data == (DataParam)&m_itmX1 || data == (DataParam)&m_itmX2) {
      m_itmDeltaX.Invalidate();
    }
    if (data == (DataParam)&m_itmY1 || data == (DataParam)&m_itmY2) {
  ...
  // Clicked on Find
  if (code == ToWord('l', 'e') &&
      data == (DataParam)&m_wndListCursor.m_proFind) {
    Find(m_wndListCursor.m_pMarker, m_wndListCursor.m_modeFind);
    return;
  }
}

void CWndMenuCursor::Find(CSettings::Marker *pMarker,
                          CSettings::Marker::EFind mode) {
