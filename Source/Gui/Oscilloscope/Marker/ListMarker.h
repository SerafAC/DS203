...
public:
  ...
  void Create(CSettings::Marker *pMarker, CWnd *pParent) {
    m_pMarker = pMarker;
    CListBox::Create(pMarker->strFullName, WsVisible | WsModal,
                     CRect(100, 30, 300, 201), pMarker->u16Color, pParent);

    if (pMarker->Type == CSettings::Marker::_Time) {
      m_proMode.Create((const char **)CSettings::Marker::ppszTextMode,
                       (NATIVEENUM *)&pMarker->Mode,
                       CSettings::Marker::_ModeMaxTime);
    } else {
      m_proMode.Create((const char **)CSettings::Marker::ppszTextMode,
                       (NATIVEENUM *)&pMarker->Mode,
                       CSettings::Marker::_ModeMaxVoltage);
      m_proSource.Create((const char **)CSettings::Marker::ppszTextSource,
                         (NATIVEENUM *)&pMarker->Source,
                         CSettings::Marker::_SourceMax);
    ...
    m_proDisplay.Create((const char **)CSettings::Marker::ppszTextDisplay,
                        (NATIVEENUM *)&pMarker->Display,
                        CSettings::Marker::_DisplayMax);
    ...
    m_itmValue.Create("Real value    0.000",
