...
public:
  ...
  CPoint operator+(const CPoint &cp) { return CPoint(x + cp.x, y + cp.y); }
};
...
public:
  ...
  int CenterX() { return (left + right) >> 1; }
  CPoint Center() { return CPoint((left + right) >> 1, (top + bottom) >> 1); }
  ...
  bool IsValid() const { return right > left; }
...
public:
  ...
  int GetMaxSize() { return m_nMaxCount; }
