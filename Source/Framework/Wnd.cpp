...
CWnd *CWnd::GetParent() { return m_pParent; }

CWnd *CWnd::GetFocus() { return m_pFocus; }
...
/*virtual*/ void CWnd::OnMessage(CWnd *pSender, CodeParam code,
                                 DataParam data) {}
...
void CWnd::SetFocus() { m_pFocus = this; }

ui8 CWnd::HasFocus() { return this == m_pFocus; }

CWnd *CWnd::GetActiveWindow() { return m_pFocus; }
...
void CWnd::prvSendMessage(CWnd *pTarget, CodeParam code, DataParam data) {
...
CWnd *CWnd::_GetPrevActiveWindow() {
  CWnd *pWnd = GetPrev();
  while (pWnd &&
         (!(pWnd->m_dwFlags & WsVisible) || (pWnd->m_dwFlags & WsNoActivate)))
    pWnd = pWnd->GetPrev();
...
bool CWnd::IsWindow() { return m_pParent ? true : false; }
...
const CRect &CWnd::GetOverlay() { return m_rcOverlay; }

const CWnd::CModal &CWnd::GetTopModal() { return CWnd::m_arrModals.GetLast(); }
