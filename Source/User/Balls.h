...
class CBallGenerator {
 public:
  ...
  void Render(ui16 *pPixels, ui32 nColor) {
    Render(pPixels, GetColorR(nColor), GetColorG(nColor), GetColorB(nColor),
           230);
  }
  ...
  void Render(ui16 *pPixels, int r, int g, int b, int a) {
    // color, light, shadow
    // r, g, b, a
    si16 pMaterial[10] = {r, 240 - r, -60, g, 240 - g, -60, b, 240 - b, -60, a};
  ...
  void render(ui16 *pPixels, si16 *mat) {
    ...
    for (y = 0; y < h; y++)
      for (x = 0; x < w; x++) {
        ...
        int r = mat[0 * 3 + 0] + ((mat[0 * 3 + 1] * c) >> 8) +
                ((mat[0 * 3 + 2] * s) >> 8);
        int g = mat[1 * 3 + 0] + ((mat[1 * 3 + 1] * c) >> 8) +
                ((mat[1 * 3 + 2] * s) >> 8);
...
class CWndBalls : public CWnd {
 ...
 public:
  ...
  CWndBalls() : m_Gen(BallSize, BallSize) { bRedraw = true; }

  virtual void Create(CWnd *pParent, ui16 dwFlags) {
    CWnd::Create("CWndBalls", dwFlags | CWnd::WsTick | CWnd::WsNoActivate,
  ...
  virtual void OnPaint() {
    ...
    ui32 nColor =
        RGB32((nR & 3) * 85, ((nR >> 2 & 3)) * 85, ((nR >> 4 & 3)) * 85);
  ...
  virtual void OnMessage(CWnd *pSender, ui16 code, ui32 data) {
    ...
    if (code == ToWord('L', 'E')) {
      ...
      CUtils::StrCpy((char*)pSector, message);

      BIOS::DSK::Write( &f, pSector );
      BIOS::DSK::Close( &f, CUtils::StrLen(message) );*/
      return;
    }
  }
