...
#define FROMFIXED(a) ((a) >> 10)
  ...
  if (nFloatAvg == 0)
    ...
    if (nValue > TOFIXED(nFloatAvg))
      nFloatAvg = (nFloatAvg * nFactorCharge +
                   TOFIXED(nValue) * (256 - nFactorCharge)) /
                  256;
    else
      nFloatAvg = (nFloatAvg * nFactorDischarge +
                   TOFIXED(nValue) * (256 - nFactorDischarge)) /
                  256;
  }
  ...
  if (bSignal) {
    ...
    if (nFloatMin == 0)
      ...
      nFloatMin =
          (nFloatMin * nFactorMin + TOFIXED(nValue) * (256 - nFactorMin)) / 256;
  }
  ...
  if (!bSignal && bLastSignal) {
    if (nCounter >= 3 && nCounter < 15) {
      ...
      BIOS::LCD::Printf(8, 30, RGB565(ffffff), RGB565(000000),
                        "len=%d (%dms), snr=%d, total=%d   ", nCounter,
                        nTick - nRising, FROMFIXED(nSnr), nTotal);

      float fReal = nTotal / 1600.0f;
      BIOS::LCD::Printf(8, 30 + 16, RGB565(ffffff), RGB565(000000),
                        "Energy = %3f kWh  ", fReal);
      ...
      if (nTick - nLastTick >= 60000) {
        ...
        float fPulsePerSecond =
            (float)(nTotal - 1 - nLastTickImp) * 1000.0f / (nTick - nLastTick);
        float fPulsePerHour = fPulsePerSecond * 3600.0f;

        // 1600 imp => 1kWh
        // 1600 imp/hour = 1kW
        float fPower = fPulsePerHour / 1600.0f;
        BIOS::LCD::Printf(8, 30 + 2 * 16, RGB565(ffffff), RGB565(000000),
                          "\x7f Minute = %3f kW (%dp)  ", fPower,
                          nTotal - 1 - nLastTickImp);
      ...
      int nTick = BIOS::SYS::GetTick();
      BIOS::LCD::Printf(40, 40, RGB565(ff0000), RGB565(000000),
                        "invalid signal! len=%d (%dms)  ", nCounter,
                        nTick - nRising);
    }
  }
  ...
  BIOS::LCD::Bar(x, BIOS::LCD::LcdHeight - 128, x + 1, BIOS::LCD::LcdHeight,
                 bSignal ? RGB565(808080) : RGB565(202020));
  BIOS::LCD::PutPixel(x, BIOS::LCD::LcdHeight - FROMFIXED(nFloatAvg) / 2 - 1,
                      RGB565(00ff00));
  BIOS::LCD::PutPixel(x, BIOS::LCD::LcdHeight - FROMFIXED(nFloatAvg) / 2 - 2,
                      RGB565(00ff00));
  BIOS::LCD::PutPixel(x, BIOS::LCD::LcdHeight - FROMFIXED(nFloatMin) / 2 - 1,
                      RGB565(0000b0));
  BIOS::LCD::PutPixel(x, BIOS::LCD::LcdHeight - FROMFIXED(nFloatMin) / 2 - 2,
                      RGB565(0000b0));
  if (nSnr > 0) {
    BIOS::LCD::PutPixel(x, BIOS::LCD::LcdHeight - FROMFIXED(nSnr) / 2 - 1,
                        RGB565(ff00ff));
    BIOS::LCD::PutPixel(x, BIOS::LCD::LcdHeight - FROMFIXED(nSnr) / 2 - 2,
                        RGB565(ff00ff));
  }
  BIOS::LCD::PutPixel(x, BIOS::LCD::LcdHeight - nLastVal / 2 - 1,
                      RGB565(ffff00));
...
void CWndEMeter::OnPaint() {
  ...
  BIOS::LCD::Bar(0, BIOS::LCD::LcdHeight - 128 - 18, BIOS::LCD::LcdWidth,
                 BIOS::LCD::LcdHeight - 128, clr);
