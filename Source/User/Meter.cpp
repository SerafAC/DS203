...
void CWndUserMeter::OnPaint() {
  ...
  BIOS::LCD::Printf(
      8, 220, RGB565(808080), RGB565(ffffff),
      "adc=%3f cal=%3f var=%3f range=%s ", m_fAverage, fCorrect, m_fVariance,
      CSettings::AnalogChannel::ppszTextResolution[Settings.CH1.Resolution]);

  if (m_fAverage < 16 || m_fAverage > 240) CUtils::StrCpy(strDisplay, " Err");
  ...
  for (; i < 4; i++) DrawDigit(20 + i * 90, 40, 0, cClr, cClr);
}

void DrawDigitH0(int x, int y, ui16 clr) {
  if (clr == RGB565(ff00ff)) return;
...
void CWndUserMeter::DrawDigit(int x, int y, int nDigit, ui16 clrOn,
                              ui16 clrOff) {
  ui32 decoder[] = {0x1111110, 0x0110000, 0x1101101, 0x1111001, 0x0110011,
                    0x1011011, 0x1011111, 0x1110000, 0x1111111, 0x1111011};
  ui32 dec = 0;
  if ((nDigit & 127) >= '0' && (nDigit & 127) <= '9') {
    dec = decoder[nDigit & 15];
    if (nDigit & 128) dec |= 0x10000000;
  } else {
    switch (nDigit) {
      case ' ':
        dec = 0;
