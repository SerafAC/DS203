...
// LINKERSECTION(".gbios")
void /*CWndUserManager::*/ FlashData(CBufferedReader2 &f,
                                     Elf32_Shdr &elfSection) {
  if (Verify(f, elfSection)) {
    BIOS::DBG::Print("Ignoring %d bytes at %08x.", elfSection.size,
                     elfSection.addr);
    return;
  }
  BIOS::DBG::Print("Flashing %d bytes at %08x.", elfSection.size,
                   elfSection.addr);

#ifndef _WIN32
  if ((elfSection.addr >> 24) == 0x20) {
    FlashRam(f, elfSection);
  } else if ((elfSection.addr >> 24) == 0x08) {
...
// LINKERSECTION(".gbios")
void /*CWndUserManager::*/ FlashBss(CBufferedReader2 &f,
                                    Elf32_Shdr &elfSection) {
  ...
  BIOS::DBG::Print("Filling %d bytes at %08x.", elfSection.size,
                   elfSection.addr);
#ifndef _WIN32
  if ((elfSection.addr >> 24) == 0x20) {
...
void CWndUserManager::ElfExecute(char *strName) {
  /*
          all variables used in this routine + (used by BIOS functions) must be
     placed
          at memory location not colliding with loaded image! Same limitation
     apply to the
  ...
  */
  ...
  fw >> CStream(&elfHeader, sizeof(Elf32_Ehdr));
  int nStringTableSectionOfs =
      elfHeader.shoff + elfHeader.shtrndx * sizeof(Elf32_Shdr);
  ...
  */
  ...
  const char *arrSecNames[] = {
      "none",    ".text",     ".data",       ".bss",    ".plt",
      ".got",    ".dynamic",  ".dynstr",     ".dynsym", ".rel.plt",
      ".interp", ".shstrtab", ".init_array", ".strtab", ".symtab"};
