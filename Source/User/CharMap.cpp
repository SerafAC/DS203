...
void DrawBig(int _x, int _y, int s, int ch) {
  ...
  for (int y = 0; y < 14; y++) {
    ...
    for (int x = 0; x < 8; x++) {
      ...
      BIOS::LCD::Bar(_x + x * s, _y + y * s, _x + (x + 1) * s, _y + (y + 1) * s,
                     clr);
    }
  }
}
...
void UpdateChar(int nPos, bool bSel) {
  ...
  if (bSel) {
    ...
    BIOS::LCD::Printf(80, 34 + 16 * 0, RGB565(ffffff), 0, "Decimal     = %d  ",
                      nPos);
    BIOS::LCD::Printf(80, 34 + 16 * 1, RGB565(ffffff), 0,
                      "Hexadecimal = 0x%02x", nPos);
    BIOS::LCD::Printf(80, 34 + 16 * 2, RGB565(ffffff), 0, "Binary      = %s",
                      CUtils::itoa2(nPos));
  }
}
