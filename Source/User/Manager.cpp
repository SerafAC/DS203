...
void CWndUserManager::InitFileList() {
  ...
  TLoadedModule *arrModules =
      (TLoadedModule *)(pCommonBuffer + nDataLength / 2);
...
/*static*/ int CWndUserManager::CompareFile(BIOS::FAT::TFindFile &fA,
...
void CWndUserManager::OnPaint() {
  ...
  BIOS::LCD::Print(4, y, RGB565(ffff00), RGB565(0000b0),
                   "     Name       Size     Date    Time");
...
void CWndUserManager::DrawLine(BIOS::FAT::TFindFile &fileInfo, int y,
                               bool bSelected) {
  ...
  if (fileInfo.nDate != 0 && fileInfo.nDate != dwJpaDate) {
    BIOS::DBG::sprintf(strAux, "%02d/%02d/%02d", (fileInfo.nDate >> 5) & 15,
                       (fileInfo.nDate) & 31, (fileInfo.nDate >> 9) - 20);
    BIOS::LCD::Print(4 + 22 * 8 + 4, y, clr, clrBack, strAux);
    BIOS::DBG::sprintf(strAux, "%02d:%02d", (fileInfo.nTime >> 11),
                       (fileInfo.nTime >> 5) & 63);
  ...
  if (nKey == BIOS::KEY::KeyEnter) {
    if (m_arrFiles[nSelected].nAtrib & BIOS::FAT::EDirectory) {
      ...
      Exec(m_strCurrentPath, m_arrFiles[nSelected].strName,
           m_arrFiles[nSelected].nFileLength);
    }
  }
}
...
void CWndUserManager::Exec(char *strPath, char *strFile, int nLength) {
  ...
  if (eType == ENone) {
    MainWnd.m_wndMessage.Show(this, "Manager", "Unknown file suffix",
                              RGB565(FF0000));
    return;
  }
    ...
    if (ElfGetInterpreter(strFullName, strLoader)) {
      ...
      if (CheckModuleConflict(dwBegin, dwEnd)) {
        ...
        if (!ElfLoad(strFullName)) {
          MainWnd.m_wndMessage.Show(this, "Manager", "Failed to execute!",
                                    RGB565(FF0000));
        } else {
          CCookies::SetCookie((char *)"gui.manager.last", strFullName);
          Settings.Save();
          BIOS::SYS::Execute(dwEntry);
          // on win32 it continues... file buffer was corrupted by linear
          // flashing
          Invalidate();
        }
      } else
        MainWnd.m_wndMessage.Show(
            this, "Manager", "Module conflict, won't load", RGB565(FFFF00));
    }
  }
  if (eType == EHex) {
    ui32 dwEntry, dwBegin, dwEnd;
    ...
    if (CheckModuleConflict(dwBegin, dwEnd)) {
      ...
      MainWnd.m_wndMessage.Show(this, "Manager", "Module conflict, won't load",
                                RGB565(FFFF00));
  }
  if (eType == EAdr) {
    ui32 dwBegin, dwEnd;
    ...
    if (FpgaLoad(strFullName)) {
      MainWnd.m_wndMessage.Show(this, "Manager", "Image flashed successfully",
                                RGB565(FFFF00));
      AddModule(strFile, nLength, (ui32)-1, dwBegin, dwEnd);
      SaveModuleList();
    } else
      MainWnd.m_wndMessage.Show(this, "Manager", "Error flashing",
                                RGB565(FF0000));
  }
  if (eType == EExe) {
#ifdef _WINDOWS
...
#else
    MainWnd.m_wndMessage.Show(this, "Manager", "Cannot execute Win32 app",
                              RGB565(FF0000));
#endif
  }
}
...
void CWndUserManager::AddModule(char *strName, int nLength, ui32 dwEntry,
                                ui32 dwBegin, ui32 dwEnd) {
  for (int i = 0; i < m_arrLoaded.GetSize(); i++)
...
bool CWndUserManager::IsModuleLoaded(char *strName, int nLength, ui32 dwEntry,
                                     ui32 dwBegin, ui32 dwEnd) {
  for (int i = 0; i < m_arrLoaded.GetSize(); i++) {
    TLoadedModule &mod = m_arrLoaded[i];
    if (strcmp(mod.strFileName, strName) == 0 &&
        (int)mod.dwFileLength == nLength && mod.dwBegin == dwBegin &&
        mod.dwEnd == dwEnd && mod.dwEntry == dwEntry) {
      return true;
    }
  }
  return false;
...
bool CWndUserManager::CheckModuleConflict(ui32 dwBegin, ui32 dwEnd) {
  ...
  for (int i = 0; i < m_arrLoaded.GetSize(); i++) {
    ...
    if (!(mod.dwEnd < dwBegin || mod.dwBegin > dwEnd)) {
      // conflicting record
      // we should ask the user whether he/she is sure to overwrite existing
      // module
