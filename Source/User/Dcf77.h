...
public:
  ...
  int BcdToInt(int nBcd) { return (nBcd & 15) + (nBcd / 16) * 10; }
  ...
  virtual void OnTimer() {
    ...
    if (m_bRedraw) {
      ...
      } DcfData[] = {// id, startbit index, length, valid value, type
      ...
      for (int i = 0; i < (int)COUNT(DcfData); i++) {
        ...
        bool bCurrent = (m_nBitIndex - 1) >= DcfItem.nStart &&
                        (m_nBitIndex - 1) < DcfItem.nStart + DcfItem.nLength;
      ...
      BIOS::LCD::Printf(20, 240 - 32 - 8, RGB565(505050), 0, "BitIndex=%d ",
                        m_nBitIndex);
      int nSeconds = m_nBitIndex > 0 ? m_nBitIndex : 0;

      if (m_lDcfAcquired == 0) {
        BIOS::LCD::Printf(20, 240 - 48 - 8, RGB565(b0b0b0), 0,
                          "%02d:%02d:%02d %s %d. %s %d      ", nHours, nMinutes,
                          nSeconds, GetWeekday(nWDay), nDay, GetMonth(nMonth),
                          nYear);
      }

      int nPassedTick = BIOS::SYS::GetTick() - nTick;
      BIOS::LCD::Printf(320, 0x10, RGB565(505050), 0, "upd %d ms  ",
                        nPassedTick);
    }
    ...
    if (/*m_bRedraw &&*/ m_lDcfAcquired != 0) {
      ...
      BIOS::LCD::Printf(20, 240 - 48 - 8, RGB565(ffffff), 0,
                        "%02d:%02d:%02d %s %d. %s %d      ", nHours, nMinutes,
                        nSeconds, GetWeekday(nWDay), nDay, GetMonth(nMonth),
                        nYear);
    }
