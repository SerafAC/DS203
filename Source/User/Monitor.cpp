...
void CWndUserMonitor::Create(CWnd *pParent, ui16 dwFlags) {
  CWnd::Create("CWndCharMap", dwFlags | CWnd::WsTick, CRect(300, 20, 400, 240),
               pParent);
  m_proBaud.Create((const char **)m_ppszTextBaud, (NATIVEENUM *)&m_nBaud,
                   COUNT(m_ppszTextBaud) - 1);
...
void CWndUserMonitor::OnTimer() {
  ...
  nCounter++;
  BIOS::LCD::Print(m_x * 8 + 4, m_y * 16 + 24,
                   (nCounter & 1) ? RGB565(ffffff) : RGB565(000000),
                   RGB565(000000), "_");
}
...
void CWndUserMonitor::OnMessage(CWnd *pSender, ui16 code, ui32 data) {
  ...
  if (pSender == &m_btnClear && code == CWnd::WmKey &&
  ...
  /*
  if ( pSender == &m_btnSend && code == CWnd::WmKey && data ==
  BIOS::KEY::KeyEnter )
  {
