...
class CWndZoomView : public CWnd {
 ...
 public:
  ...
  virtual void OnPaint() {
    DrawWave();
    DrawProgress();
    BIOS::LCD::Bar(0, m_rcClient.bottom - 20, 400, m_rcClient.bottom,
                   RGB565(808080));
  }
  ...
  void DrawWave(int nPixelLeft = 0, int nPixelRight = 400) {
    ...
    if (nViewEnd - nViewBegin + 1 > 400) {
      ...
      for (int x = nPixelStart; x < nPixelRight; x++) {
        ...
        int nFixIndex =
            x * (nViewEnd - nViewBegin + 1) * 256 / 400 + nViewBegin * 256;
    ...
    if (nPixelLeft == 0)
      BIOS::LCD::Printf(0, m_rcClient.bottom - 16, RGB565(ffffff),
                        RGB565(808080), "view %d-%d   ", nViewBegin, nViewEnd);
  }
