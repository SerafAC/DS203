...
public:
  ...
  void Clear() { memset(m_arrCounts, 0, 60); }
  ...
  virtual void OnPaint() {
    ...
    // 0.186 => 0.09
    BIOS::LCD::Printf(20, 30, RGB565(ffffff), RGB565(000000), "Count = %d   ",
                      GetSum());
    BIOS::LCD::Printf(20, 30 + 16, RGB565(ffffff), RGB565(000000),
                      "HPM = %f   ", (float)fHPS);
    BIOS::LCD::Printf(20, 30 + 32, RGB565(ffffff), RGB565(000000),
                      "Dose rate = %f "
                      "\xE6"
                      "Sv/h  ",
                      (float)fRate);
    ...
    for (int j = 0; j < 2; j++) {
      ...
      BIOS::LCD::Print(20 + (i % 10) * 30, 100 + (i / 10) * 16, RGB565(ffffff),
                       i == nSlot ? RGB565(0000b0) : RGB565(000000), num);
    }
