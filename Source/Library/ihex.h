...
enum IHexErrors {
  ...
  IHEX_ERROR_FILE =
      -1, /**< Error code for error while reading from or writing to a file. You
             may check errno for the exact error if this error code is
             encountered. */
  IHEX_ERROR_EOF = -2, /**< Error code for encountering end-of-file when reading
                          from a file. */
  IHEX_ERROR_INVALID_RECORD =
      -3, /**< Error code for error if an invalid record was read. */
  IHEX_ERROR_INVALID_ARGUMENTS = -4, /**< Error code for error from invalid
                                        arguments passed to function. */
  IHEX_ERROR_NEWLINE = -5, /**< Error code for encountering a newline with no
                              record when reading from a file. */
};
...
typedef struct {
  ...
  uint8_t data[IHEX_MAX_DATA_LEN / 2]; /**< The 8-bit array data field, which
 ...
 * \param ihexRecord A pointer to the target Intel HEX8 record structure where
 * these fields will be set.
 * \return IHEX_OK on success, otherwise one of the IHEX_ERROR_ error codes.
 * \retval IHEX_OK on success.
 * \retval IHEX_ERROR_INVALID_ARGUMENTS if the record pointer is NULL, or if the
 ...
 * \param ihexRecord A pointer to the Intel HEX8 record structure that will
 * store the read record.
 * \param in A FIL pointer to an opened FIL that can be read.
 * \return IHEX_OK on success, otherwise one of the IHEX_ERROR_ error codes.
 * \retval IHEX_OK on success.
 * \retval IHEX_ERROR_INVALID_ARGUMENTS if the record pointer or FIL pointer is
 * NULL.
...
int Read_IHexRecord(IHexRecord *ihexRecord, CBufferedReader2 &in) {
  ...
  /* Copy the ASCII hex encoding of the count field into hexBuff, convert it to
   * a usable integer */
  ...
  /* Copy the ASCII hex encoding of the address field into hexBuff, convert it
   * to a usable integer */
  ...
  /* Loop through each ASCII hex byte of the data field, pull it out into
   * hexBuff,
  ...
  /* Copy the ASCII hex encoding of the checksum field into hexBuff, convert it
   * to a usable integer */
