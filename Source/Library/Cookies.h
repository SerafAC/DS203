...
public:
	...
	static void SetCookie(char* strName, char* strValue)
	{
		...
		_ASSERT( (int)((pCookie ? BIOS::UTIL::StrLen(pCookie) : 0) + BIOS::UTIL::StrLen(strName) + BIOS::UTIL::StrLen(strValue)) + 2 < nLength );
		...
		if ( nNewLength < nOldLength )
		{
			...
			for (int i=pCookie-strBuffer+nOldLength; i<=(int)BIOS::UTIL::StrLen(strBuffer); i++)
				strBuffer[i-nReduce] = strBuffer[i];
		} else if ( nNewLength > nOldLength )
		{
			int nExpand = nNewLength - nOldLength;
			for ( int i=BIOS::UTIL::StrLen(strBuffer); i>=pCookie-strBuffer+nOldLength; i--)
