...
class CPin {
 ...
 public:
  ...
  void Float() {
    // with pull up
    BIOS::GPIO::SetState(m_nPort, m_nPin,
                         BIOS::GPIO::StateInput | BIOS::GPIO::StateInputPull);
    BIOS::GPIO::SetPin(m_nPort, m_nPin, true);
  }
 ...
 public:
  ...
  CPin(int nPortPin) { Create(nPortPin); }
  ...
  operator const int() { return Get(); }
  operator const ui8() { return Get(); }
};
