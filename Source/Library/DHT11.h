...
/*
        Usage:
        // connect device to UART-TX pin, no pullup necessary when the cable is
   short
        ...
        calling GetDewPoint cause device to freeze, probably the math functions
   are not linked
*/
...
class CDHT11 : public CPin, CDelay {
 ...
 public:
  ...
  int GetHumidity() { return m_buffer[0]; }

  int GetTemperature() { return m_buffer[2]; }
  ...
  float GetDewPoint() {
    ...
    float temp = (a * GetTemperature()) / (b + GetTemperature()) +
                 log(GetHumidity() / 100.0f);
