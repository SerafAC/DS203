...
/*static*/ const float CSettings::TimeBase::pfValueResolution[] = {
    ...
    10e-6f, 20e-6f, 50e-6f, 100e-6f, 200e-6f, 500e-6f, 1e-3f, 2e-3f, 5e-3f,
    10e-3f, 20e-3f, 50e-3f, 100e-3f, 200e-3f, 500e-3f, 1.0f};

/*static*/ const char *const CSettings::Generator::ppszTextWave[] = {
    "DC",       "Sin HQ", "Sin LQ",  "Triangle",
    "Sawtooth", "Square", "Cardiac", "Volatile"};
...
/*static*/ const char *const CSettings::Marker::ppszTextMode[] = {"Off", "On",
                                                                  "Auto"};
/*static*/ const char *const CSettings::Marker::ppszTextSource[] = {"CH1",
                                                                    "CH2"};
/*static*/ const char *const CSettings::Marker::ppszTextDisplay[] = {"Raw",
                                                                     "Units"};
/*static*/ const char *const CSettings::Marker::ppszTextFind[] = {
    "Minimum", "Average", "Maximum"};

/*static*/ const char *const CSettings::Measure::ppszTextEnabled[] = {"Off",
                                                                      "On"};
/*static*/ const char *const CSettings::Measure::ppszTextSource[] = {
    "CH1", "CH2", "Math"};
/*static*/ const char *const CSettings::Measure::ppszTextType[] = {
    "Minimum",  "Maximum",  "Average",  "RectAvg", "RMS",
    "Vpp",      "Freq",     "Period",   "PWM %",   "Delta+M",
    "Angle+M",  "Time H",   "Time L",   "Rising",  "Falling",
    "FormFact", "Sigma",    "Variance", "Baud",    "P(W)+M",
    "P(kW)+M",  "Q(VAr)+M", "Q(kVA)+M", "S(VA)+M", "S(kVA)+M"};
/*static*/ const char *const CSettings::Measure::ppszTextSuffix[] = {
    "V",  "V",   "V",  "V",   "V",    "V",  "kHz", "ms", "",
    "ms", "deg", "ms", "ms",  "ms",   "ms", "",    "",   "",
    "",   "W",   "kW", "VAr", "kVAr", "VA", "kVA"};
...
void CSettings::Reset() {
  ...
  // cant use CWndToolBar::Find, MainWnd has been not initialized!
...
#define _COPY(type, target, ...)                           \
  ...
  //{-20, 280} );
  //	_COPY( si32, CH1Calib.CalData[AnalogChannel::_200mV].m_arrCurveQout,
  //{9380*5, -154816*5} );
  //	_COPY( si16, CH1Calib.CalData[AnalogChannel::_200mV].m_arrCurveKin,
  //{-20, 15, 75, 90, 245, 280} );
  //	_COPY( si32, CH1Calib.CalData[AnalogChannel::_200mV].m_arrCurveKout,
  //{581*5, 580*5, 581*5, 582*5, 584*5, 580*5} );
}

void CSettings::Kick() { m_lLastChange = BIOS::SYS::GetTick(); }
...
void CSettings::SaveCalibration() {
  ...
  CStream bufStream(pSharedBuffer, FILEINFO::SectorSize);
  bufStream << dwId << CStream(&DacCalib, sizeof(DacCalib));
