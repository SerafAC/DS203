...
public:
  ...
  virtual void OnPaint() {
    ...
    if (HasFocus()) {
      ...
      if (m_pszId)
        BIOS::LCD::Print(m_rcClient.left + 12, m_rcClient.top, RGB565(000000),
                         RGBTRANS, m_pszId);
    } else {
  ...
  virtual void OnPaint() {
    if (HasFocus()) {
      ...
      if (m_pImage) {
        ...
        bmp.Blit(m_rcClient.right - bmp.m_width,
                 m_rcClient.bottom - bmp.m_height);
      }
      BIOS::LCD::Print(m_rcClient.left + 12, m_rcClient.top, RGB565(000000),
                       RGBTRANS, m_pszId);
    } else {
...
public:
  ...
  virtual VPNavigate operator+(si8 dir) {
    _ASSERT(0);
    return Disabled;
  }
...
public:
  ...
  virtual void OnPaint() {
    ...
    if (prev == CValueProvider::Yes)
      ...
      BIOS::LCD::Draw(x - 8, m_rcClient.top, RGB565(808080), RGBTRANS,
                      CShapes::more_left);
    ...
    if (next == CValueProvider::Yes)
      x += BIOS::LCD::Draw(x, m_rcClient.top, clr, RGBTRANS,
                           CShapes::more_right);
    else if (next == CValueProvider::No)
      x += BIOS::LCD::Draw(x, m_rcClient.top, RGB565(808080), RGBTRANS,
                           CShapes::more_right);
  }

  virtual void OnKey(ui16 nKey) {
    if (nKey & BIOS::KEY::KeyLeft &&
        (*m_pProvider) - 1 == CValueProvider::Yes) {
      (*m_pProvider)--;
      Invalidate();
...
public:
  ...
  virtual void OnPaint() {
    ...
    BIOS::LCD::Draw(m_rcClient.right - 3 - 6, m_rcClient.top + 20,
                    RGB565(000000), RGBTRANS, CShapes::more_right);
    ...
    BIOS::Print( rcClient.CenterX()-((ui8)BIOS::UTIL::StrLen(m_pszId)<<2),
    rcClient.top,
            RGB565(000000), RGBTRANS, m_pszId);

    BIOS::Draw( m_rcClient.left + 4, m_rcClient.top+18, RGB565(000000),
    RGBTRANS, more_left );
    BIOS::Draw( m_rcClient.right - 3 - 8, m_rcClient.top+18, RGB565(000000),
    RGBTRANS, more_right );
    ...
    */
    ...
    while ((nWidth = m_pProvider->GetWidth()) < rcClient.Width()) {
      ...
      if (bFocus) {
        ...
        BIOS::LCD::Draw(rcClient.left - 8, rcClient.top, RGB565(000000),
                        RGBTRANS, CShapes::sel_left);
      }
      ...
      if (bFocus)
        BIOS::LCD::Draw(rcClient.left, rcClient.top, RGB565(000000), RGBTRANS,
                        CShapes::sel_right);
  ...
  virtual void OnKey(ui16 nKey) {
    ...
    if (nKey & BIOS::KEY::KeyRight &&
        (*m_pProvider) + 1 != CValueProvider::No) {
      (*m_pProvider)++;
      Invalidate();
    }
    if (nKey & BIOS::KEY::KeyEnter) {
...
public:
  ...
  virtual void OnPaint() {
    ...
    BIOS::LCD::Print(rcItem.left, rcItem.top, RGB565(000000), RGBTRANS,
                     m_pszId);
    rcItem.top += 16;
    if (HasFocus()) {
      rcItem.left -= 8;
      rcItem.left += BIOS::LCD::Draw(rcItem.left, rcItem.top, RGB565(0000000),
                                     RGBTRANS, CShapes::sel_left);

      ui16 nWidth = m_pProvider->GetWidth();
      BIOS::LCD::Bar(rcItem.left, rcItem.top, rcItem.left + nWidth,
                     rcItem.top + 14, RGB565(000000));

      m_pProvider->OnPaint(rcItem, 1);
      rcItem.left += nWidth;
      rcItem.left += BIOS::LCD::Draw(rcItem.left, rcItem.top, RGB565(0000000),
                                     RGBTRANS, CShapes::sel_right);
...
public:
  ...
  virtual void OnPaint() {
    ...
    if (HasFocus())
      BIOS::LCD::Draw(m_rcClient.left - 7, m_rcClient.top, clr, RGBTRANS,
                      CShapes::more_left);
...
public:
  void Create(const char *pszId, ui16 dwFlags, CWnd *pParent,
    ...
    for (int i = 0; i < N; i++) {
      ...
      m_wndItem[i].Create("child", CWnd::WsVisible, rcChild, arrProviders[i],
                          this);
    }
  }
  void OnPaint() {
...
public:
  ...
  virtual VPNavigate operator+(si8 d) { return Yes; }

  virtual void operator++(int) { (*m_pNumber) += m_nStep; }
  ...
  virtual void OnPaint(const CRect &rcRect, ui8 bFocus) {
    ...
    char strDigit[2] = {(char)('0' + nDigit), 0};
