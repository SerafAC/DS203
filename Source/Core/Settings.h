...
public:
  ...
  public:
    ...
    enum eEnabled { _NO = 0, _YES = 1, _EnabledMax = _YES } Enabled;
    enum eCoupling { _AC = 0, _DC, _GND, _CouplingMax = _GND } Coupling;
    enum eResolution {
      _50mV,
      _100mV,
      _200mV,
    ...
    enum eProbe { _1X = 0, _10X, _100X, _1000X, _ProbeMax = _1000X } Probe;
    ...
    virtual CSerialize &operator<<(CStream &stream) {
      stream << _E(Enabled) << _E(Coupling) << _E(Resolution) << _E(Probe)
             << u16Color << u16Position;
      return *this;
    }
    virtual CSerialize &operator>>(CStream &stream) {
      stream >> _E(Enabled) >> _E(Coupling) >> _E(Resolution) >> _E(Probe) >>
  ...
  public:
    ...
    ui16 u16Color;
    si16 u16Position;
    enum { _POS = 0, _NEG, _PolarityMax = _NEG } Polarity;
  ...
  public:
    ...
    enum ESync { _Auto, _Norm, _Single, _Scan, _None, _SyncMax = _None } Sync;
    enum {
      _EdgeHL,
      _EdgeLH,
      _LevelLow,
      ...
      _LowerDTHigh,
      _GreaterDTHigh,
      _TypeMax = _GreaterDTHigh
    } Type;
    enum { _CH1, _CH2, _CH3, _CH4, _Math, _SourceMax = _Math } Source;
    enum { _Run, _Stop, _Wait, _Unsync, _StateMax = _Unsync } State;
    si16 nLevel;
    si16 nTime;
    si16 nHoldOff;
    si16 nPosition;
    ...
    virtual CSerialize &operator<<(CStream &stream) {
      stream << _E(Sync) << _E(Type) << _E(Source) << _E(State) << nLevel
             << nTime << nHoldOff;
      return *this;
    }
    virtual CSerialize &operator>>(CStream &stream) {
  ...
  public:
    ...
    enum { _Off, _On, _MaxEnabled = _On } Enabled;
    enum ESource { _CH1, _CH2, _Math, _MaxSource = _Math } Source;
    enum {
      _Min,
      _Max,
      _Avg,
    ...
    enum ERange { _View, _Selection, _All, _MaxRange = _All } Range;
  ...
  public:
    ...
    enum { _TY, _XY, _YX, _AxesMax = _YX } Axes;
    enum { _Points, _Lines, _Fill, _DrawMax = _Fill } Draw;
    enum { _AvgNo, _AvgCh1, _AvgCh2, _AverageMax = _AvgCh2 } Average;
    enum { _PerNo, _PerYes, _PersistMax = _PerYes } Persist;
    enum { _GridNone, _GridDots, _GridLines, _GridMax = _GridLines } Grid;
    enum { _AxisNone, _AxisSingle, _AxisDouble, _AxisMax = _AxisDouble } Axis;
    ...
    virtual CSerialize &operator>>(CStream &stream) {
      stream >> _E(Axes) >> _E(Draw) >> _E(Average) >> _E(Persist) >>
          _E(Grid) >> _E(Axis);
      return *this;
    }
  };
  ...
  public:
    ...
    enum { _Rectangular, _Hann, _WindowMax = _Hann } Window;
    enum { _Fft, _FftTime, _Spectrograph, _DisplayMax = _Spectrograph } Display;
    enum { _Lin, _Log, _ScaleMax = _Log } YScale;
    enum { _Off, _SrcCh1, _SrcCh2, _SourceMax = _SrcCh2 } MarkerSource;
    enum { _Manual, _FindMax, _ModeMax = _FindMax } MarkerMode;
    ...
    virtual CSerialize &operator<<(CStream &stream) {
      stream << _E(Window) << _E(Display) << _E(YScale) << _E(MarkerSource)
             << nMarkerX << _E(MarkerMode);
      return *this;
    }
    virtual CSerialize &operator>>(CStream &stream) {
      stream >> _E(Window) >> _E(Display) >> _E(YScale) >> _E(MarkerSource) >>
  ...
  public:
    ...
    enum { _On, _Off, _BeepMax = _Off } m_Beep;
    ...
    virtual CSerialize &operator<<(CStream &stream) {
      stream << m_nMenuItem << m_nUptime << m_nBacklight << m_nVolume
             << m_nStandby << _E(m_Beep) << m_nShortcutTriangle << m_nShortcutS1
             << m_nShortcutS2 << m_bUartTest;
      return *this;
    }
    virtual CSerialize &operator>>(CStream &stream) {
      stream >> m_nMenuItem >> m_nUptime >> m_nBacklight >> m_nVolume >>
...
public:
  ...
  virtual CSerialize &operator<<(CStream &stream) {
    ...
    stream << dwId << Runtime << CH1 << CH2 << CH3 << CH4 << Time << Trig << Gen
           << MarkT1 << MarkT2 << MarkY1 << MarkY2 << Meas[0] << Meas[1]
           << Meas[2] << Meas[3] << Meas[4] << Meas[5] << MathA << MathB
           << MathC << Math << Spec << dwEnd;
  ...
  virtual CSerialize &operator>>(CStream &stream) {
    ...
    if (dwId == _VERSION) {
      stream >> Runtime >> CH1 >> CH2 >> CH3 >> CH4 >> Time >> Trig >> Gen >>
          MarkT1 >> MarkT2 >> MarkY1 >> MarkY2 >> Meas[0] >> Meas[1] >>
          Meas[2] >> Meas[3] >> Meas[4] >> Meas[5] >> MathA >> MathB >> MathC >>
          Math >> Spec >> dwEnd;
