...
public:
  ...
  virtual void operator++(int) { (*m_pVal)++; }
  ...
  virtual ui32 Get() { return (ui32)*m_pVal; }
};
...
public:
  ...
  virtual void OnPaint(const CRect &rcRect, ui8 bFocus) {
    ...
    x += BIOS::LCD::Print(x, rcRect.top, RGBTRANS, clr, CUtils::clrhex(clr));
    x += BIOS::LCD::Draw(x, rcRect.top, clr, RGBTRANS, CShapes::sel_right);
  }

  virtual ui16 GetWidth() { return 3 * 8; }
};
...
public:
  ...
  virtual ui16 GetWidth() { return 0; }

  virtual ui32 Get() { return 0; }
};
...
public:
  ...
  virtual void OnPaint(const CRect &rcRect, ui8 bFocus) {
    ...
    BIOS::LCD::Print(rcRect.left, rcRect.top, clr, RGBTRANS, temp);
  }

  virtual ui16 GetWidth() { return 8; }
};
...
public:
        void Create(const char* pszId, const char** ppszText, NATIVEENUM* val,
NATIVEENUM max, CWnd* pParent)
        {
                m_ppszText = ppszText;
                m_pVal = val;
                m_Max = max;
        ...
        virtual void OnPaint()
        {
                ...
                x += BIOS::Print( x, m_rcClient.top, clr, RGBTRANS,
                        ((const char**)m_ppszText)[ *m_pVal ]);
                x += BIOS::Draw( x, m_rcClient.top, clrright, RGBTRANS,
more_right );
        }
...
public:
        void Create(const char* pszId, ui16* val, ui16 _min, ui16 _max, CWnd*
pParent)
        {
                m_pVal = val;
                m_nMin = _min;
...
public:
        void Create(const char* pszId, ui16* pColor, EComponent Component, CWnd*
pParent)
        {
                m_pColor = pColor;
                m_Component = Component;
