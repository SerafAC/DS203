...
// input_storage_class, output_storage_class, input_variabe, output_variable,
// internal_calculations, length
template <class Ttabin, class Ttabout, class Tvarin, class Tvarout, class Tcalc,
          int N_>
...
public:
  ...
  typedef CalibrationCurve<float, si16, float, int, float, ePoints>
      Interpolator;
...
public:
  ...
  typedef CalibrationCurve<si16, si32, int, int, int, LinCalibCurve::eKPoints>
      InterpolatorK;
  typedef CalibrationCurve<si16, si32, int, int, int, LinCalibCurve::eQPoints>
      InterpolatorQ;
  ...
  void Prepare(AnalogChannel *pSource, FastCalc &fast) {
    ...
    fast.K = InterpolatorK::Get(pCurCurve.m_arrCurveKin,
                                pCurCurve.m_arrCurveKout, nVert);
    fast.Q = InterpolatorQ::Get(pCurCurve.m_arrCurveQin,
                                pCurCurve.m_arrCurveQout, nVert);
  ...
  static void Prepare(int nRes, int nVert, LinCalibCurve &pCurCurve,
                      FastCalc &fast) {
    const static float arrMultipliers[AnalogChannel::_ResolutionMax + 1] = {
        50e-3f, 100e-3f, 200e-3f, 500e-3f, 1.0f, 2.0f, 5.0f, 10.0f};
  ...
  int Correct(FastCalc &fast, int nAdc) {
    // return -fast.Zero + ( ((fast.Q + nAdc * fast.K)*5) >> (16-5) ); //
    // *32/200mV 5=1/200mV
    // K, Q by mali byt v rozmere pixelov (0..255) reprezentujuce vysku od
    // 0..8divov a nie vo voltoch!
  ...
  float Voltage(FastCalc &fast, float fAdc) {
    ...
    // pre ziskanie skutocneho napatia teda vydelime osmimi a prenasobime
    // konstantou
  ...
  float Correct(FastCalc &fast, float fAdc) {
    ...
    // return -fZero + ( ((fast.Q + fAdc * fast.K)*5.0f) / 2048.0f ); //
    // *32/200mV 5=1/200mV   2048 = 65536 {fixed 16.16} / 32 {1 div}
    return -fZero + ((fast.Q + fAdc * fast.K) / 65536.0f / 32.0f);

    // *32/0.2
  }
#endif
  int GetZero(FastCalc &fast) { return -fast.Zero; }
