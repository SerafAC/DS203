...
DECLARE_FUNCTION(_About) {
  BIOS::SERIAL::Send("DS203 terminal service v2.0\n"
...
DECLARE_FUNCTION(_Help) {
  ...
  for (; pTokens->m_pszToken; pTokens++) {
    ...
    case CEvalToken::PrecedenceLow:
      BIOS::SERIAL::Send("Low-Func");
      break;
    case CEvalToken::PrecedenceFunc:
      BIOS::SERIAL::Send("Function");
      break;
    case CEvalToken::PrecedenceConst:
      BIOS::SERIAL::Send("Constant");
      break;
    case CEvalToken::PrecedenceVar:
      BIOS::SERIAL::Send("Variable");
      break;
    default:
      BIOS::SERIAL::Send("Unknown ");
      break;
    }
    BIOS::SERIAL::Send("  ");
...
static void _ScanWindowRecursive(CWnd *pWnd, int nMask, char *msg) {
  // {ptr:0x01234567, name:"MainWnd", style:8,
  // rect:{left:20,top:10,right:30,bottom:40}, children:[...] }
  BIOS::SERIAL::Send("{");
...
DECLARE_FUNCTION(_MenuItem) {
  class CVariable : public CEval::CEvalVariable {
    virtual void Set(CEval::CEvalOperand &pOperand) {
      MainWnd.SendMessage(&MainWnd.m_wndToolBar, ToWord('g', '2'),
                          pOperand.GetInteger());
    }
    virtual CEval::CEvalOperand Get() {
...
DECLARE_FUNCTION(_LcdGetBitmap) {
  ...
  CRect rcTransfer(arrOperands[-7].GetInteger(), arrOperands[-5].GetInteger(),
                   arrOperands[-3].GetInteger(), arrOperands[-1].GetInteger());
...
DECLARE_DYNAVAR(ui32, _IoGpioACrl,
                *BIOS::GPIO::GetRegister(BIOS::GPIO::PortA, BIOS::GPIO::RegCrl))
DECLARE_DYNAVAR(ui32, _IoGpioACrh,
                *BIOS::GPIO::GetRegister(BIOS::GPIO::PortA, BIOS::GPIO::RegCrh))
DECLARE_DYNAVAR(ui32, _IoGpioAIdr,
                *BIOS::GPIO::GetRegister(BIOS::GPIO::PortA, BIOS::GPIO::RegIdr))
DECLARE_DYNAVAR(ui32, _IoGpioAOdr,
                *BIOS::GPIO::GetRegister(BIOS::GPIO::PortA, BIOS::GPIO::RegOdr))
DECLARE_DYNAVAR(ui32, _IoGpioABsrr,
                *BIOS::GPIO::GetRegister(BIOS::GPIO::PortA,
                                         BIOS::GPIO::RegBsrr))
DECLARE_DYNAVAR(ui32, _IoGpioALckr,
                *BIOS::GPIO::GetRegister(BIOS::GPIO::PortA,
                                         BIOS::GPIO::RegLckr))
